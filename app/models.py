...
class SearchableMixin:
    ...
    def search(cls, expression, page, per_page):
        ...
        for i, id in enumerate(ids):
            when.append((id, i))
        # db.case([:param: list of condition and result tuples],
        #     [:param: value = column of the condition])
        # This makes the query result from the Post ordered same as ids
        ...
        if current_app.config['TESTING']:
            for obj in session._changes['add']:
                if isinstance(obj, SearchableMixin):
                    add_to_index(obj.__tablename__+'_test', obj)
            for obj in session._changes['update']:
                if isinstance(obj, SearchableMixin):
                    add_to_index(obj.__tablename__+'_test', obj)
            for obj in session._changes['delete']:
                if isinstance(obj, SearchableMixin):
                    remove_from_index(obj.__tablename__+'_test', obj)
        else:
            for obj in session._changes['add']:
                if isinstance(obj, SearchableMixin):
    ...
    def reindex(cls):
        if current_app.config['TESTING']:
            create_index(cls.__tablename__+'_test')
