...
def explore():
    ...
    next_url = url_for('main.explore', page=posts.next_num) \
        if posts.has_next else None
    prev_url = url_for('main.explore', page=posts.next_num) \
        if posts.has_prev else None
    return render_template(
...
@login_required
def search():
    if not (g.search_form.validate() and current_app.elasticsearch.ping()):
        flash('Search failed. Redirected to Explore page.')
    ...
    return render_template('search.html', title='Search', posts=posts,
        total=total, per_page=current_app.config['POSTS_PER_PAGE'],
        next_url=next_url, prev_url=prev_url)
...
def messages():
    ...
    return render_template(
        'messages.html', title='Messages', messages=messages.items,
        next_url=next_url, prev_url=prev_url)
